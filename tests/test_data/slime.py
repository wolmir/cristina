...
class Slime(pygame.sprite.Sprite):
    def __init__(self, ROM, x, y, startFacing):
        ...
        self.ground = 1 # assume we are on the ground, this will trigger events
        self.fallNoise = load_sound('fall3.wav')
        self.dieNoise = load_sound('roar1.wav')
        self.absorbTime = 90 # how long does it take to absorb this?
        self.selfWorth = 500 # contemplate this
    ...
    def setSprite(self,num,auto_flip=1):
        ...
        if num != self.spriteNum or (auto_flip==1 and self.facing != self.autoFlip):
            ...
            self.image.blit(self.ROM, (0,0), Rect(num*(TILE_SIZE*2)+self.angry*TILE_SIZE*8,TILE_SIZE*8,TILE_SIZE*2,TILE_SIZE*2))
    ...
    def releaseStun(self):
        if self.ground == 1: # unstun on the ground
            ...
            self.aiTick = 0 # reset our AI ticker
            self.setSprite(0) # basic sprite
            
    def setAngry(self):
        self.angry = 1
        self.moveX = 0.85 # FAST
            
    def takeDamage(self, game, projName):
        if projName == "slimeBall" or projName == "spikeMine" or projName == "flameBall":
