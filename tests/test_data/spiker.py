...
class Spiker(pygame.sprite.Sprite):
    def __init__(self, ROM, x, y, startFacing):
        ...
        self.moveX = 0.35 # not as fast as slimes
        self.maxY = 3.0
        self.angry = 0
        ...
        self.absorbTime = 120 # how long does it take to absorb this?
        self.selfWorth = 750 # contemplate this
    ...
    def setSprite(self,num,auto_flip=1):
        ...
        if num != self.spriteNum or (auto_flip==1 and self.facing != self.autoFlip):
            self.autoFlip = self.facing # we want to update a sprite if the facing has changed
            self.spriteNum = num
            self.image.fill(COLOR_KEY) # blank the sprite
            self.image.blit(self.ROM, (0,0), Rect(num*(TILE_SIZE*2)+self.angry*TILE_SIZE*8,TILE_SIZE*10,TILE_SIZE*2,TILE_SIZE*2))
    ...
    def setFall(self):
        ...
        # play on a specific channel so it overrides other slimes
    ...
    def setAngry(self):
        self.angry = 1
        self.moveX = 0.55 # FAST
            
    def takeDamage(self, game, projName):
        if projName == "slimeBall":
            self.aiState = 2 # curl up into protective shell
            self.vel = (0.0, self.vel[1]) # don't move! 
            self.setSprite(2) # hide!
            pygame.mixer.Channel(2).play(self.bounceNoise)
        elif projName == "spikeMine" or projName == "flameBall":
